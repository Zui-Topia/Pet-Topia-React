...
const StepButton = styled.button`
  background-color: ${(props) =>
    props.disabled
      ? "#d3d3d3"
      : "#000000"}; // 비활성화 시 회색, 활성화 시 검정색
  border-radius: 10px; // 테두리 반경
  border: 0px solid #000000; // 테두리
  cursor: ${(props) =>
    props.disabled
      ? "not-allowed"
      : "pointer"}; // 비활성화 시 'not-allowed' 커서, 활성화 시 'pointer' 커서
  height: 70px; // 고정 높이
  width: 450px; // 고정 너비
...
const StepButtonText = styled.div`
  color: ${(props) =>
    props.disabled ? "#a9a9a9" : "#ffffff"}; // 비활성화 시 회색, 활성화 시 흰색
  font-family: "Kanit-Regular", Helvetica; // 글꼴
  font-size: 20px; // 글자 크기
  font-weight: 400; // 글자 굵기
`;
...
const Reservation = () => {
  ...
  useEffect(() => {
    const fetchStrollerData = async () => {
      try {
        const response = await ReservationAPI.petStrollerCnt(
          selectedBranchId,
          selectedDate
        );
